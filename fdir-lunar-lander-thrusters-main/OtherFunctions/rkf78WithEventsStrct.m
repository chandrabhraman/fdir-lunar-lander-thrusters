function [xout,tout,StopIntegration] = rkf78WithEventsStrct (deq, neq, ti, tf, h, tetol, x,MaxStep,EventsFunction,ReqStop,StopTolerance,EventsEnabled,NSG,Other,ENV,FunPosVelInput)
...
while (1)
   ...
   if (abs(ti - tf) < 0.00000001 || StopIntegration==1)
      xout = x;
      tout = ti;
%       StoreTime=StoreTime';
      return;
   ...
   xdot = feval(deq, ti, x,NSG,Other,ENV,FunPosVelInput);
   ...
   if (xerr > 1)
      ...
      % twrk - Previous Time Step
      % ti - Current Time Step
      % x - Current States
      % xwrk - Previous States
      if(EventsEnabled==1)
        [StatesFromZeroCrossing,TimeZeroCrossing,StopIntegration]=zerocrossing(EventsFunction,[twrk ti],[xwrk x],neq,tetol,MaxStep,deq,StopTolerance,ReqStop,NSG,Other,ENV,FunPosVelInput);
        if(StopIntegration==1)
            x=StatesFromZeroCrossing;
            ti = TimeZeroCrossing;
        end
      end
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
